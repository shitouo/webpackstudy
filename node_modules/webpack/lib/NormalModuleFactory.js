...
class NormalModuleFactory extends Tapable {
	constructor(context, resolverFactory, options) {
		...
		this.ruleSet = new RuleSet(options.defaultRules.concat(options.rules)); // 格式化rule
		this.cachePredicate =
		...
		this.hooks.factory.tap("NormalModuleFactory", () => (result, callback) => {
			...
			resolver(result, (err, data) => {
				...
				this.hooks.afterResolve.callAsync(data, (err, result) => {
					...
					console.log('afterResolve result =======', result);

					let createdModule = this.hooks.createModule.call(result);
					
					if (!createdModule) {
						console.log('没有createdModule');
						if (!result.request) {
					...
					console.log('createModule---------', createdModule);
		...
		this.hooks.resolver.tap("NormalModuleFactory", () => (data, callback) => {
			...
			console.log('loaderResolver', loaderResolver)
			const normalResolver = this.getResolver("normal", data.resolveOptions);
			// console.log('normalResolver', normalResolver)
			...
			asyncLib.parallel(
				...
				(err, results) => {
					...
					resource = results[1].resource; // 此处的resource为绝对地址了
					console.log('resource', resource);
					...
					for (const r of result) {
						if (r.type === "use") {
							if (r.enforce === "post" && !noPrePostAutoLoaders) {
								...
								console.log('要给useLoaders添加loader')
					...
					asyncLib.parallel(
						...
						(err, results) => {
							console.log('results2222222222', results);
							if (err) return callback(err);
							console.log('matchResource', matchResource);
		...
		this.hooks.beforeResolve.callAsync(
			...
			(err, result) => {
				...
				factory(result, (err, module) => {
					...
					console.log('this.cachePredicate(module)', this.cachePredicate(module));
